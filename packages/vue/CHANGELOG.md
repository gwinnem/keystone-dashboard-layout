...
- **`mitt` — the last remaining runtime dependency.** This package now
  has **zero runtime dependencies**, matching what `2.0.0`'s own
  changelog entry already claimed after `interact.js` was removed (an
  inaccuracy in that entry, corrected here rather than left standing).
  `mitt` was used for exactly one thing — the internal `GridLayout`
  ↔`GridItem` pub/sub event bus (`TGridLayoutEventBus`/
  `TGridItemEventBus`) — and only its narrow `on`/`off`/`emit` surface,
  never wildcard (`'*'`) listeners or its exposed `.all` map. Replaced
  with a small hand-rolled typed emitter
  (`@keystone-dashboard-layout/core`'s new `createEventEmitter()`/
  `IEventEmitter<Events>`, in `helpers/event-emitter.ts`) implementing
  exactly that surface. `mitt`'s own `Emitter<T>` type was never part
  of the public API (neither event-bus type is exported from either
  package's barrel), so this is a purely internal implementation
  change — no consumer-facing impact, no migration needed. See
  `NOTICE.md` for the updated third-party dependency accounting.
...
- **`zIndex` prop and `#header` slot (`GridItem`)** — found via a direct
  ...
  its `DashboardLayoutPanel` class — see
  `PARITY_GAP_VUE_COMMERCIAL_AND_CROSS_FRAMEWORK.md`.
  - `zIndex` (`number | null`, default `null`): an explicit
    stacking-order override applied as an inline style, which always
    wins over the library's own implicit `.vue-static`/`.resizing`
    CSS-class-based z-index handling regardless of the item's current
    state — e.g. pin one panel permanently above every other panel,
    including while others are mid-resize.
  - `#header` named slot: a distinct title-bar region, rendered above
    the existing content area in a flex-column layout, only when
    actually provided — the default (no-header) case's DOM structure
    and behavior are completely unchanged, deliberately, to avoid any
    risk to existing consumers.
  
  Both tracked as `ROADMAP.md` items 33–34. Demo's "Per-item
  overrides" view gained matching controls (item "0" only).

- **`resizeHandles` prop (`GridLayout` grid-wide default, `GridItem`
  per-item override)** — restricts which of the 8 corner/edge resize
  handles actually render and activate, instead of the previous
  all-or-nothing `showResizeHandles` toggle. `null` (the `GridItem`
  default) inherits `GridLayout`'s own default (all 8, by default) —
  same inherit pattern as `isDraggable`/`isResizable`/`showCloseButton`.
  An empty array (`resizeHandles: []`) is a deliberate, valid way to
  disable handle-based resizing for an item without setting
  `isResizable: false` entirely (keyboard shift+arrow resize still
  works). New `TResizeHandle` type, exported from
  `@keystone-dashboard-layout/core`. Closes the `@marsio/vue-grid-layout`
  parity gap noted in `COMPARISON_ALTERNATIVES.md` and tracked as
  `ROADMAP.md` item 24. Demo's "Per-item overrides" view gained a live
  checkbox control per handle.
