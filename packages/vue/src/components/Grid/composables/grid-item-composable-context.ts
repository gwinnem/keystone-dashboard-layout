...
import { RESIZE_EDGE_MAP, TResizeHandle } from '@/core/helpers/native-interaction';
...
export interface IGridItemComposableContext {
  ...
  /** The resolved set of active resize handles (`GridItem`'s own `resizeHandles` prop, or `GridLayout`'s default when it's `null`) — `useGridItemResize`'s `tryMakeResizable` only wires up native resize listeners on handles present in this set. */
  resizeHandles: Ref<TResizeHandle[]>;
