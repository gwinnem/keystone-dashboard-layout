...
export default defineConfig({
  ...
  //      was unset, rather than emitting a wrong one.
  //   3. `<meta property="og:url">` — omitted for the identical reason,
  //      confirmed in the same source.
  // `public/robots.txt`'s own Sitemap: line uses this same domain — keep
  ...
  integrations: [
    ...
    starlight({
      ...
      favicon: '/favicon.svg',
      // Route data middleware adding og:image/twitter:image meta tags
      // (pointing at the per-page images generated by
      // src/pages/og/[...slug].ts, via astro-og-canvas) and JSON-LD
      // structured data (TechArticle, BreadcrumbList, and
      // SoftwareSourceCode on the three framework home pages) to every
      // page — both entirely missing before this was added.
      //
      // Implemented as a `Head` component override (below, in
      // `components:`), NOT as `routeMiddleware` — that option doesn't
      // exist on the actually-installed @astrojs/starlight@0.30.6
      // (confirmed via a real, failed `astro build` run: "Unrecognized
      // key(s) in object: 'routeMiddleware'" — that option was only
      ...
      components: {
        ...
        // header comment for why this is a `Head` override rather than
        // `routeMiddleware`.
          ...
          items: [
            ...
            },
            {
              ...
              items: [
                ...
                { label: '53 — Resize direction toggles', slug: 'vue/examples/resize-direction-toggles' },
              ],
            },
          ],
        },
          ...
          items: [
            ...
            },
            {
              ...
              items: [
                ...
                { label: '53 — Resize direction toggles', slug: 'react/examples/resize-direction-toggles' },
              ],
            },
          ],
        },
