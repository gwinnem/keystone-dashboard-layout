  ...
  vite: {
    ...
    resolve: {
      alias: [
        ...
        //
        // Paths updated to `packages/vue/src/...` — this project has
        // since been restructured into a `packages/{vue,react,core}`
        // monorepo; the original `../../src/...` paths (correct when the
        // source lived directly at the repo root, before that
        // restructure) were stale and resolved to a `src/` directory
        // that no longer exists there at all, confirmed via a real
        // "failed to resolve import" error from an actual example page.
        { find: /^vue-ts-responsive-grid-layout$/, replacement: path.resolve(__dirname, '../../packages/vue/src/components/index.ts') },
        // Same rationale as the main alias above — the `/core` entry
        // point now lives in its own separate `packages/core` package,
        // not a `core/` subfolder under `packages/vue/src` (confirmed:
        // no such subfolder exists there), so this points there instead.
        { find: /^vue-ts-responsive-grid-layout\/core$/, replacement: path.resolve(__dirname, '../../packages/core/src/index.ts') },
        // The three below mirror packages/vue/vite.config.js's own
        // `resolve.alias` exactly (confirmed by reading that file
        // directly, not assumed) — `components/index.ts` (aliased just
        // above) itself imports internally via `@/core/...` and
        // `@/components/Grid/layout-definition`, so this docs site needs
        // the exact same alias set that file's own real build already
        // relies on, or those internal imports fail to resolve here even
        // though the entry point itself does. Order matters (Vite's own
        // alias matching stops at the first hit): the two specific
        // entries must come before the generic `@` catch-all, exactly as
        // in that file. Plain strings, not regex, deliberately -- Vite's
        // own string-alias matching (unlike a `^...$`-anchored regex,
        // which would only match the bare key itself) matches the key as
        // a *prefix* followed by `/`, which is what makes
        // `@/core/common/interfaces/aria-labels.interface` (a longer
        // subpath, not the bare `@/core` string) actually resolve.
        { find: '@/core', replacement: path.resolve(__dirname, '../../packages/core/src') },
        { find: '@/components/Grid/layout-definition', replacement: path.resolve(__dirname, '../../packages/core/src/layout-definition') },
        { find: '@', replacement: path.resolve(__dirname, '../../packages/vue/src') },
      ],
    },
  },
});
